...
namespace sym2 {
    class ChildIterator : public boost::stl_interfaces::proxy_iterator_interface<ChildIterator,
        ...
        /* Proxy iterator that is able to traverse through both logical and physical children. Can
         * be constructed with static functions, for an expression with children, or treating a
         * single expression as a single child. As the step size can vary when constructed from an
         * expression with children, this can only be a forward iterator (as
      ...
      public:
        ...
        /* Creates an iterator over all physical children of the given expression. Example: passing
         * a complex number will create an iterator over real and complex part. Passing a large
         * rational will iterate over numerator and denomiator. For composites with logical
         * children, the behaviour is identical to iterators created with logicalChildren(...); */
        ...
        /* Necessary because the above operator++ hides the one in the base class, as mentioned in
         * the docs. */
        using boost::stl_interfaces::proxy_iterator_interface<ChildIterator,
          std::forward_iterator_tag, ExprView<>>::operator++;
